...
def help():
    ...
    print('output   -o      determina el formato de salida `print(pinta en pantalla)` o `file(lo almacena en un archivo)` ')
    print('odir     -d     determina el directorio de almacenar las nanopublicaciones, solo si el output es `file`')
    print('\n')
...
def test(inputfile, outputFormat='print', rdfFormat='trig', outputdir=None):
    ...
    with open(inputfile) as infile:
        ...
        print("\t-->", len(annotations),
              "annotaciones encontradas desde el archivo")
        print("\t-->", "presentando datos basicos de la anotaciones")
        for annotation in annotations:
            print("\t-->", type(annotation),
                  annotation.id, "tags:", annotation.tags)
        # paso 2 split
        ...
        print("pass 3) -> ", ("Almacenando anotaciones en "+outputdir)
              if outputFormat == 'file' else "Imprimiendo en pantalla anotaciones")
        if outputFormat == 'file' and outputdir != None and not os.path.exists(outputdir):
        ...
        for request in requests:
            ...
            if outputFormat == 'file':
                ...
                print("\t-->", "Almacenando nanopublicacion en",
                      "'"+filepath + "'")
